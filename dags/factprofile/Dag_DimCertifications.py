...
def generate_next_entreprise_pk():
...
def insert_entreprises(ti):
    ...
    cur.execute("SELECT COUNT(*) FROM public.dim_entreprise;")
    count_existing = cur.fetchone()[0]
    counter = count_existing + 1
    total = 0
    ...
    for nom, lieu in entreprises:
        ...
        entreprise_pk = generate_next_entreprise_pk()
        codeentreprise = generate_entreprise_code(counter)
        counter += 1
