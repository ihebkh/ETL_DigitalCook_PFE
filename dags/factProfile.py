...
def insert_data_to_postgres(client_fk, competencegenerales_fk, language_fk, interests_fk, preferedjoblocations_fk,
                            etude_fk, visa_fk, projet_fk, contact_fk, permis_fk, certification_pk, experience_fk):
    conn = get_postgres_connection()
    cur = conn.cursor()

    try:
        # Insert data into fact_client_profile table
        cur.execute("""
        ...
        conn.commit()
        print(f"Data inserted for client_fk: {client_fk}")
...
# Updated match_and_display_factcode_client_competence_interest to call insert_data_to_postgres
...
def match_and_display_factcode_client_competence_interest():
    ...
    for user in mongo_data_list:
        ...
        # Get permisConduire from the profile and simpleProfile sections
        ...
        # Process experiences (both profile and simpleProfile)
        if experiences:
        ...
        # Process certifications (both profile and simpleProfile)
        if certifications:
        ...
        if simpleProfile_certifications:
            for certification in simpleProfile_certifications:
                if isinstance(certification, dict):  # If certification is a dictionary
                    certification_name = certification.get("nomCertification", "").strip()
                    certification_pk = get_certification_pk_from_postgres(certification_name)
                    if certification_pk:
        ...
        for i in range(max_length):
            ...
            # Call insert function to insert data to PostgreSQL
            insert_data_to_postgres(client_fk, competence_fk, language_fk, interest_pk, job_location_pk,
                                    study_level_fk, visa_pk, project_pk, contact_pk, permis_fk, certification_pk, experience_fk)
