...
def query_dim_influencer_for_name(pg_cursor, name, last_name):
    if not name or not last_name:
        return None
    pg_cursor.execute(
        "SELECT user_pk FROM public.dim_user WHERE nom = %s AND prenom = %s",
        (name, last_name)
    )
...
def get_date_pk_from_date(pg_cursor, date_value):
    ...
    pg_cursor.execute("SELECT date_pk FROM public.dim_dates WHERE datecode = %s", (date_str,))
    result = pg_cursor.fetchone()
    return result[0] if result else None
...
def upsert_fact_dossier(
    ...
    offre_emploi_step4, offre_etude_step4, service_nom, service_prix, service_discount, service_extra,created_at, updated_at
):
    pg_cursor.execute("""
        INSERT INTO public.fact_dossier (
            ...
            service_nom, service_prix, service_discount, service_extra,created_at, updated_at
        )
